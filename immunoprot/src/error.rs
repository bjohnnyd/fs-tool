...
impl std::fmt::Display for HLAError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        match self {
            HLAError::UnknownLigandGroup(lg_group) => writeln!(f, "Could not determine ligand group from '{}'\nAcceptable groups are: {}\n{}", lg_group, MHCI_LIGAND_GROUPS, help_string),
            HLAError::UnknownExpressionChangeTag(exprs) => writeln!(f, "Unknown expression change tag '{}', acceptable values are {}\n{}", exprs, EXPRESSION_CHANGES, help_string),
            HLAError::CouldNotParseMHCI(mhc_I) => writeln!(f, "Could not parse MHCI allele from '{}'\n Colons (:) are needed in MHCI names while other separators and placeholders are optional.\n{}", mhc_I,  help_string),
            HLAError::GeneUnknown(mhc_I) => writeln!(f, "Could not determine gene from '{}'. Recognized HLA genes are {}.\n{}", mhc_I, HLA_GENES, help_string),
            HLAError::NoAlleleGroup(mhc_I) => writeln!(f, "Could not parse allele group information{}\n{}", mhc_I, help_string),
        }
    }
}
